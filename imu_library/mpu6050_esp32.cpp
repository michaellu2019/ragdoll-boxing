...
MPU6050::MPU6050(int a) // Uses I2C communication by default
{
  MPU6050_ADDRESS = a;
  Wire.begin();
}
...
bool MPU6050::setupIMU(uint8_t timeout)
{
  uint32_t total = timeout*1000;
  bool good = false;
  uint32_t start = millis();
  while((millis()-start < total)){
    Serial.printf("%d\n\n\n",readByte(MPU6050_ADDRESS, WHO_AM_I_MPU6050));
    if (readByte(MPU6050_ADDRESS, WHO_AM_I_MPU6050) == 0x68 || readByte(MPU6050_ADDRESS, WHO_AM_I_MPU6050) == 0x69){ //was 0x73 for mpu6050
      good = true;
	Serial.println("inside");
      break;
    }
  }
  Serial.println("Good initialized");
  if (good){
    initMPU6050();
    Serial.println("Initialized IMU");
    getAres(); //call this so the IMU internally knows its range/resolution
    getGres(); //call this so the IMU internally knows its range/resolution
    //getMres(); 
    return true;
  }else{
    return false;
...
void MPU6050::initMPU6050()
{
  ...
  // With the MPU6050, it is possible to get gyro sample rates of 32 kHz (!),
  // 8 kHz, or 1 kHz
  ...
  writeByte(MPU6050_ADDRESS, GYRO_CONFIG, c );
  ...
  writeByte(MPU6050_ADDRESS, ACCEL_CONFIG, c);
  ...
  writeByte(MPU6050_ADDRESS, ACCEL_CONFIG2, c);
...
void MPU6050::calibrateMPU6050(float * gyroBias, float * accelBias)
{
  ...
  writeByte(MPU6050_ADDRESS, PWR_MGMT_1, READ_FLAG);
  delay(100);
  ...
  // Apparently this is not working for the acceleration biases in the MPU-6050
...
// deviation is a pass.
void MPU6050::MPU6050SelfTest(float * destination)
{
  ...
  writeByte(MPU6050_ADDRESS, GYRO_CONFIG, 1<<FS);
  // Set accelerometer rate to 1 kHz and bandwidth to 92 Hz
  writeByte(MPU6050_ADDRESS, ACCEL_CONFIG2, 0x02);
  // Set full scale range for the accelerometer to 2 g
  writeByte(MPU6050_ADDRESS, ACCEL_CONFIG, 1<<FS);
  ...
  writeByte(MPU6050_ADDRESS, ACCEL_CONFIG, 0xE0);
  ...
  selfTest[0] = readByte(MPU6050_ADDRESS, SELF_TEST_X_ACCEL);
  // Y-axis accel self-test results
  selfTest[1] = readByte(MPU6050_ADDRESS, SELF_TEST_Y_ACCEL);
  // Z-axis accel self-test results
  selfTest[2] = readByte(MPU6050_ADDRESS, SELF_TEST_Z_ACCEL);
  // X-axis gyro self-test results
  selfTest[3] = readByte(MPU6050_ADDRESS, SELF_TEST_X_GYRO);
  // Y-axis gyro self-test results
  selfTest[4] = readByte(MPU6050_ADDRESS, SELF_TEST_Y_GYRO);
  // Z-axis gyro self-test results
  selfTest[5] = readByte(MPU6050_ADDRESS, SELF_TEST_Z_GYRO);
...
void MPU6050::magCalMPU6050(float * bias_dest, float * scale_dest)
{
...
uint8_t MPU6050::writeByte(uint8_t deviceAddress, uint8_t registerAddress,uint8_t data)
{
...
uint8_t MPU6050::readByte(uint8_t deviceAddress, uint8_t registerAddress)
{
...
uint8_t MPU6050::readBytes(uint8_t deviceAddress, uint8_t registerAddress, uint8_t count, uint8_t * dest)
{
  // Initialize the Tx buffer
  Wire.beginTransmission(deviceAddress);
  // Put slave register address in Tx buffer
